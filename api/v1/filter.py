...
# """router method for fetching all the secotrs avalaible"""
...
def get_categories(sector: str = Query(..., description="The sector to filter categories by")):
    categories = list(set(item['category']
                      for item in data if item['sector'] == sector))
    if not categories:
...
def get_regions(source: str = Query(..., description="The source to filter regions by"),
                category: str = Query(...,
                                      description="The category to filter sources by"),
                sector: str = Query(..., description="The sector to filter categories by")):
    regions = list(set(item['region'] for item in data if (item['source'] == source) and (
        item['category'] == category) and (item['sector'] == sector)))
    if not regions:
        raise HTTPException(status_code=404, detail="Sector not found")
    return regions
...
def get_years(region: str = Query(..., description="The region to filter years by"),
              category: str = Query(...,
                                    description="The category to filter sources by"),
              sector: str = Query(...,
                                  description="The sector to filter categories by"),
              source: str = Query(..., description="The source to filter years by")):
    years = list(set(item['year'] for item in data if (item['region'] == region) and (
        item['source'] == source) and (item['category'] == category) and (item['sector'] == sector)))
    if not years:
        raise HTTPException(status_code=404, detail="Sector not found")
    return years
...
def get_data(
        ...
        category: str = Query(...,
                              description="The category to filter data by"),
    ...
    data_list = list(item for item in data if (item['sector'] == sector) and (item['category'] == category) and (
        item['source'] == source) and (item['region'] == region) and (item['year'] == year))
